...
\subsection{Logistic Regression}
By performing a grid search across the hyperparameter ranges specified in the methodology, we record the best hyperparameters in Table 1:
\begin{table}[h!]
\centering
...
\hline
\end{tabular}
\caption{Best hyperparameters from cross-validation for TF-IDF and No-Text logistic regression models. Accuracy is macro-averaged over 5-fold CV.}
\label{tab:best-hyperparameters}
\end{table}
The mean evaluation metrics for the best combinations of hyperparameters are summarized in Table 2.
