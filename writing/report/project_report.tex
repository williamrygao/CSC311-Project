...
\subsection{Logistic Regression}
By performing a grid search across the hyperparameter ranges specified in the methodology, we found the best hyperparameters to be:
\begin{itemize}
  \item \texttt{max\_features} $= 300$.
  \item \texttt{ngram\_range} $= (1,1)$.
  \item $C = 5$.
  \item \texttt{penalty} $= L2$.
  \item \texttt{solver} $=$ saga.
\end{itemize}
The mean evaluation metrics for the best combinations of hyperparameters are summarized in Table 1.

\textbf{Analysis:} Incorporating TF-IDF features improved validation performance, increasing all evaluation metrics by roughly 3-4 points. Training accuracy for the TF-IDF variant was substantially higher ($\approx$96\%), suggesting some degree of overfitting, though the validation scores remained higher than the non-text model. The confusion matrices indicate that most misclassifications occur between \textit{Claude} and \textit{Gemini}, likely reflecting subtle differences in responses that are challenging to capture with the current features.

\textbf{Final Model Choice and Test Performance:} Based on cross-validation, the logistic regression model using TF-IDF features with $C=1$, L2 regularization, and the \texttt{lbfgs} solver achieved the best mean F1 score of 0.691. We estimate that this model will achieve approximately \textbf{69\% accuracy} on unseen test data, justified by consistent performance across all folds and stable cross-validation metrics.
